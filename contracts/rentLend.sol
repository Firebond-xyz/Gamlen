...
import "./interfaces/IRentLend.sol";

contract RentLend is IRentLend, ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    struct Lending {
        IRentLend.NFTStandard nftStandard;
    ...
    modifier onlyAdmin {
        require(msg.sender == admin, "MetaBorrow::not admin");
        _;
    }

    modifier notPaused {
        require(!paused, "MetaBorrow::paused");
        _;
    }
    ...
    function bundleCall(function(CallData memory) _handler, CallData memory _cd)
        private
    {
        require(_cd.nfts.length > 0, "MetaBorrow::no nfts");
        while (_cd.right != _cd.nfts.length) {
            if (
                (_cd.nfts[_cd.left] == _cd.nfts[_cd.right]) &&
                (_cd.nftStandard[_cd.right] == IRentLend.NFTStandard.E1155)
            ) {
                _cd.right++;
            } else {
                _handler(_cd);
    ...
    function lend(
        IRentLend.NFTStandard[] memory _nftStandard,
    ...
    function distributePayments(
        ...
        require(
            ...
            "MetaBorrow::total payment wo collateral is zero"
        );
        require(sendLenderAmt > 0, "MetaBorrow::lender payment is zero");
        ...
        private
    {
        ...
        require(maxRentPayment > 0, "MetaBorrow::collateral plus rent is zero");
    ...
    function safeTransfer(
        ...
        if (_cd.nftStandard[_cd.left] == IRentLend.NFTStandard.E721) {
            IERC721(_cd.nfts[_cd.left]).transferFrom(
                _from,
                _to,
                _cd.tokenIds[_cd.left]
            );
        } else if (_cd.nftStandard[_cd.left] == IRentLend.NFTStandard.E1155) {
            ...
            revert("MetaBorrow::unsupported token type");
        }
    }
    ...
    function handleLend(CallData memory _cd) private {
        for (uint256 i = _cd.left; i < _cd.right; i++) {
            ...
            bool nftIs721 = (_cd.nftStandard[i] == IRentLend.NFTStandard.E721);
    ...
    function handleRent(CallData memory _cd) private {
        ...
        for (uint256 i = _cd.left; i < _cd.right; i++) {
...

            {
                ...
                require(rentPrice > 0, "MetaBorrow::rent price is zero");
                require(nftPrice > 0, "MetaBorrow ::nft price is zero");
    ...
    function ensureIsNotZeroAddr(address _addr) private pure {
        require(_addr != address(0), "MetaBorrow::zero address");
    }

    function ensureIsZeroAddr(address _addr) private pure {
        require(_addr == address(0), "MetaBorrow::not a zero address");
    }

    function ensureIsNull(Lending memory _lending) private pure {
        ensureIsZeroAddr(_lending.lenderAddress);
        require(_lending.maxRentDuration == 0, "MetaBorrow::duration not zero");
        require(_lending.dailyRentPrice == 0, "MetaBorrow ::rent price not zero");
        require(_lending.nftPrice == 0, "MetaBorrow::nft price not zero");
    }

    function ensureIsNotNull(Lending memory _lending) private pure {
        ensureIsNotZeroAddr(_lending.lenderAddress);
        require(_lending.maxRentDuration != 0, "MetaBorrow::duration zero");
        require(_lending.dailyRentPrice != 0, "MetaBorrow::rent price is zero");
        require(_lending.nftPrice != 0, "MetaBorrow::nft price is zero");
    }

    function ensureIsNull(Renting memory _renting) private pure {
        ensureIsZeroAddr(_renting.renterAddress);
        require(_renting.rentDuration == 0, "MetaBorrow::duration not zero");
        require(_renting.rentedAt == 0, "MetaBorrow::rented at not zero");
    }

    function ensureIsNotNull(Renting memory _renting) private pure {
        ensureIsNotZeroAddr(_renting.renterAddress);
        require(_renting.rentDuration != 0, "MetaBorrow::duration is zero");
        require(_renting.rentedAt != 0, "MetaBorrow::rented at is zero");
    }

    function ensureIsLendable(CallData memory _cd, uint256 _i) private pure {
        require(_cd.lentAmounts[_i] > 0, "MetaBorrow::lend amount is zero");
        require(_cd.lentAmounts[_i] <= type(uint8).max, "MetaBorrow::not uint8");
        require(_cd.maxRentDurations[_i] > 0, "MetaBorrow::duration is zero");
        require(
            _cd.maxRentDurations[_i] <= type(uint8).max,
            "MetaBorrow::not uint8"
        );
        require(
            uint32(_cd.dailyRentPrices[_i]) > 0,
            "MetaBorrow::rent price is zero"
        );
        require(uint32(_cd.nftPrices[_i]) > 0, "MetaBorrow::nft price is zero");
    }
    ...
    function ensureIsRentable(
        ...
        require(
            ...
            "MetaBorrow::cant rent own nft"
        );
        require(_cd.rentDurations[_i] <= type(uint8).max, "MetaBorrow::not uint8");
        require(_cd.rentDurations[_i] > 0, "MetaBorrow::duration is zero");
        require(
            _cd.rentDurations[_i] <= _lending.maxRentDuration,
            "MetaBorrow::rent duration exceeds allowed max"
        );
    }
    ...
    function ensureIsReturnable(
        ...
        require(_renting.renterAddress == _msgSender, "MetaBorrow::not renter");
        require(
            !isPastReturnDate(_renting, _blockTimestamp),
            "MetaBorrow::past return date"
        );
    }
    ...
    function ensureIsStoppable(Lending memory _lending, address _msgSender)
        ...
        pure
    {
        require(_lending.lenderAddress == _msgSender, "MetaBorrow::not lender");
    }
        ...
        pure
    {
        require(
            isPastReturnDate(_renting, _blockTimestamp),
            "MetaBorrow::return date not passed"
        );
    }
    ...
    function ensureIsUnpackablePrice(bytes4 _price, uint256 _scale)
        ...
        pure
    {
        require(uint32(_price) > 0, "MetaBorrow::invalid price");
        require(_scale >= 10000, "MetaBorrow::invalid scale");
    }

    function ensureTokenNotSentinel(uint8 _paymentIx) private pure {
        require(_paymentIx > 0, "MetaBorrow::token is sentinel");
    }
